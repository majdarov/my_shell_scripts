...
set_ps1() {
    ...
    local fg_myblue=$(tput setaf 27)   # My color blue
    local fg_mygrey=$(tput setaf 239)  # My color grey
    ...
    local bg_mycyan=$(tput setab 39)   # My color light cyan
    ...
    case $1 in
        ...
        deb_git)
            # Debian/Ubuntu style + git branch:
            #   \u@\h - username@host (bold/green)
            #   \w - working directory (bold/blue)
            #   \$ - prompt (# if root, otherwise $) (bold/white)
...
_git_info() {
    ...
    local fg_darkblue=$(tput setaf 234)
    ...
    if [ "${git_branch_name}" = "main" ] || [ "${git_branch_name}" = "master" ]; then
        git_info="${bg_salat}${fg_darkblue}${start_underline} ( ${git_branch_name} ) "
    else
        git_info="${bg_salat}${fg_darkblue} ( ${git_branch_name} ) "
    fi
